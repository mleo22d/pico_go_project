...
void line_follow_callback(const void * msgin) {
    ...
    if (line_follow_mode) {
        //printf("[PICO] Modo seguidor de línea ACTIVADO\n");
    }
    else {
...
void line_follow() {
    ...
    if (sensor_sum < 5000) {
        ...
        smoothed_power_diff = 0.7f * smoothed_power_diff + 0.3f * power_diff;

        // Limitar rango de corrección
        if (smoothed_power_diff > maximum) smoothed_power_diff = maximum;
        if (smoothed_power_diff < -maximum) smoothed_power_diff = -maximum;

        // Aplicar control con mínimo para evitar que se frene el motor
        int left = maximum - smoothed_power_diff;
        int right = maximum + smoothed_power_diff;

        int min_speed = 10;
        left = min(max(left, min_speed), 100);
        right = min(max(right, min_speed), 100);

        motors.set_motor(left, right);
...
int main()
{
    ...
    */
    ...
    rclc_timer_init_default(&line_follow_timer, &support, RCL_MS_TO_NS(10), line_follow_timer_callback);

    
    rclc_executor_t ros_executor;
    rclc_executor_t control_executor;
    // ROS-related executor (2 subs + 1 timer = 3)
    rclc_executor_init(&ros_executor, &support.context, 3, &allocator);

    // Control-only executor (1 handle: line_follow_timer)
    rclc_executor_init(&control_executor, &support.context, 1, &allocator);

    // ROS executor
    rclc_executor_add_timer(&ros_executor, &timer);  // ultrasónico
    rclc_executor_add_subscription(&ros_executor, &cmd_vel_subscriber, &cmd_vel_msg, cmd_vel_subscription_callback, ON_NEW_DATA);
    rclc_executor_add_subscription(&ros_executor, &line_follow_subscriber, &line_follow_msg, line_follow_callback, ON_NEW_DATA);

    // Control executor
    rclc_executor_add_timer(&control_executor, &line_follow_timer);
    ...
    {
        cyw43_arch_poll();
        rclc_executor_spin_some(&ros_executor, RCL_MS_TO_NS(1));
        rclc_executor_spin_some(&control_executor, RCL_MS_TO_NS(1));
